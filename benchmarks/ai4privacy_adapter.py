...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("Install the benchmark dependencies with: pip install -r requirements.txt") from exc

    # Resolve a mutable ref such as `main` to the immutable Hub commit SHA and
    # load that exact revision. This makes the generated 500-example benchmark
    # reproducible even if the dataset repository changes later.
    ...
    # reservoir sampler remains deterministic for this fixed revision/order.
    stream = load_dataset(
        DATASET_NAME,
        split=args.split,
        streaming=True,
