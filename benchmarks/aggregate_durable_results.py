...
def _entity(raw: dict) -> PIIEntity:
    return PIIEntity(
        ...
        source=str(raw.get("source", "durable")),
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate a durable benchmark JSONL")
    ...
    with Path(args.input).open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if record_id in rows_by_id:
                raise SystemExit(f"Duplicate document id: {record_id}")
    ...
    if args.expected_documents is not None and len(rows) != args.expected_documents:
        raise SystemExit(
            ...
            "Resume the durable runner before aggregating."
        )
    if not rows:
        raise SystemExit("No durable result rows found")
    ...
    llm_seconds = sum(float(row.get("llm_seconds", 0.0)) for row in rows)
    llm_failures = sum(int(row.get("llm_failures", 0)) for row in rows)
    method = str(rows[0].get("method", "durable"))
    ...
    summary = {
        ...
        "llm_calls": llm_calls,
        "llm_call_rate": llm_calls / len(rows),
        "llm_seconds": llm_seconds,
        "llm_failures": llm_failures,
        "ginza_available": all(bool(row.get("ginza_available", False)) for row in rows),
    }
